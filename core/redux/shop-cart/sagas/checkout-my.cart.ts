...
export function *handleCheckoutMyCurrentShopCart
  (action: ReturnType<typeof ShopCartActions.checkoutMyShopCart_request>) {
  ...
  try {
    ...
    const response: IResponse<IShopCartResponse> =
      yield call(restRequest,
                 API_SHOPCART.POST_CHECKOUT_BYTOKEN(),
                 'POST', 
                 {},
                 { 'Authorization': `Bearer ${allState.auth.credentials.token_access}` })
      
    if (response.meta.ok) {
      yield put(ShopCartActions.checkoutMyShopCart_success({
        meta: {
      ...
      yield put(ShopCartActions.checkoutMyShopCart_failed({ 
        server_status: 'error', 
        server_action: action.meta.do,
...
export function *shopCartCheckoutMyCurrent() {
  yield takeEvery(getType(ShopCartActions.checkoutMyShopCart_request), handleCheckoutMyCurrentShopCart)
}